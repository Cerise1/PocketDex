...
final class ThreadsListViewModel: ObservableObject {
    ...
    private struct CachedThreadListSnapshot: Codable {
        let cachedAt: Date
    ...
    private static let threadListCachePrefix = "pocketdex.thread-list-cache.v1."
    private static let threadListCacheMaxAge: TimeInterval = 24 * 60 * 60
    ...
    func setConfiguration(_ configuration: ServerConfiguration?) {
        ...
        if !hydrateCachedThreadList(for: configuration) {
            threads = []
            workspaceRoots = []
            serverDeviceName = nil
        }
    ...
    func refresh(
        ...
        if didLoadThreads {
            persistThreadListCache(for: configuration)
        }
    ...
    private func hydrateCachedThreadList(for configuration: ServerConfiguration) -> Bool {
        let key = Self.threadListCacheKey(for: configuration)
        guard let data = UserDefaults.standard.data(forKey: key) else {
            return false
        }
        guard let snapshot = try? JSONDecoder().decode(CachedThreadListSnapshot.self, from: data) else {
            UserDefaults.standard.removeObject(forKey: key)
            return false
        }
        if Date().timeIntervalSince(snapshot.cachedAt) > Self.threadListCacheMaxAge {
            UserDefaults.standard.removeObject(forKey: key)
            return false
        }

        threads = snapshot.threads.map(ThreadRunOptimismStore.apply(to:))
        workspaceRoots = snapshot.workspaceRoots.isEmpty
            ? Self.workspaceRootsFromThreads(snapshot.threads)
            : snapshot.workspaceRoots
        serverDeviceName = snapshot.serverDeviceName
        return !snapshot.threads.isEmpty
    }

    private func persistThreadListCache(for configuration: ServerConfiguration) {
        let snapshot = CachedThreadListSnapshot(
            cachedAt: Date(),
            threads: threads,
        ...
        UserDefaults.standard.set(data, forKey: Self.threadListCacheKey(for: configuration))
    }

    private static func threadListCacheKey(for configuration: ServerConfiguration) -> String {
        threadListCachePrefix + configuration.displayAddress
    }
