...
final class ThreadDetailViewModel: ObservableObject {
    ...
    @Published private(set) var hasOlderTurns = false
    ...
    private static let initialTurnPageSize = 8
    private static let olderTurnPageSize = 8
    ...
    func refresh(showLoader: Bool) async {
        ...
        do {
            let loadedThread = try await apiClient.readThread(
                threadID: threadID,
                config: configuration,
                turnLimit: currentThreadPageLimit
            )
    ...
    private var currentThreadPageLimit: Int {
        max(thread?.turns.count ?? 0, Self.initialTurnPageSize)
    }

    func loadOlderTurns() async {
        guard let configuration else { return }
        guard hasOlderTurns, !isLoadingOlderTurns else { return }
        guard let currentThread = thread, let firstTurnID = currentThread.turns.first?.id else { return }

        isLoadingOlderTurns = true
        defer { isLoadingOlderTurns = false }

        do {
            let olderPage = try await apiClient.readThread(
                threadID: threadID,
                config: configuration,
                turnLimit: Self.olderTurnPageSize,
                beforeTurnID: firstTurnID
            )
            let merged = mergeOlderTurnPage(olderPage, into: currentThread)
            applyThreadSnapshot(merged)
            errorMessage = nil
        } catch {
            errorMessage = error.localizedDescription
            emitStopFlowDebug(
                "load_older_turns_failed",
                actionID: activeInterruptActionID,
                detail: ["error": error.localizedDescription],
                configuration: configuration
            )
    ...
    private static func isBenignStreamTransportErrorMessage(_ message: String) -> Bool {
        let normalized = message
            .trimmingCharacters(in: .whitespacesAndNewlines)
            .lowercased()
        if normalized.isEmpty { return false }
        return normalized.contains("message too long")
    }
    ...
    private func applyThreadSnapshot(_ loadedThread: PocketDexThreadDetail) {
        ...
        hasOlderTurns = loadedThread.hasMoreTurns
    ...
    private func mergeOlderTurnPage(
        _ olderPage: PocketDexThreadDetail,
        into currentThread: PocketDexThreadDetail
    ) -> PocketDexThreadDetail {
        var seenTurnIDs = Set(olderPage.turns.map(\.id))
        let mergedTurns = olderPage.turns + currentThread.turns.filter { turn in
            seenTurnIDs.insert(turn.id).inserted
        }
        return PocketDexThreadDetail(
            id: currentThread.id,
            title: currentThread.title ?? olderPage.title,
            preview: currentThread.preview ?? olderPage.preview,
            cwd: currentThread.cwd.isEmpty ? olderPage.cwd : currentThread.cwd,
            updatedAt: currentThread.updatedAt ?? olderPage.updatedAt,
            turns: mergedTurns,
            turnsOffset: olderPage.turnsOffset,
            turnsTotal: olderPage.turnsTotal ?? currentThread.turnsTotal,
            hasMoreTurns: olderPage.hasMoreTurns,
            externalRun: currentThread.externalRun ?? olderPage.externalRun
        )
    }
    ...
    private func connectStream() {
        ...
        stream.subscribe(
            threadID: threadID,
            resume: true,
            resumeFrom: lastSyncedSeq > 0 ? lastSyncedSeq : nil,
            snapshotTurnLimit: currentThreadPageLimit
        )
    }
    ...
    private func handleStreamEvent(_ event: PocketDexStreamEvent) {
        ...
        case let .error(message):
            streamConnected = false
            if Self.isBenignStreamTransportErrorMessage(message) {
                scheduleDeferredRefresh(delayNanoseconds: 0, force: true)
                return
            }
            if detectOutOfCreditIfNeeded(message) {
        ...
        for turn in thread.turns {
            ...
            for (itemIndex, item) in turn.items.enumerated() {
                ...
                case "agentMessage":
                    ...
                    let text = Self.stripCodexAppDirectives(from: rawText)
                    if !text.isEmpty {
                        turnRows.append(
                            ConversationTimelineItem(
    ...
    private static func stripCodexAppDirectives(from text: String) -> String {
        ...
        let keptLines = normalized
            ...
            .filter { line in
                ...
                return !directivePrefixes.contains { trimmed.hasPrefix($0) }
            }
