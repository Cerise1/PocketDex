...
struct ThreadConversationView: View {
    ...
    private var conversationBody: some View {
        ConversationTimelineView(
            ...
            isLoadingOlderTurns: viewModel.isLoadingOlderTurns,
...
private struct ConversationTimelineView: View, Equatable {
    ...
    @State private var olderAutoLoadArmed = false
    @State private var olderAutoLoadRequiresReentry = false
    @State private var olderLoadAnchorID: String?
        ...
        GeometryReader { geometry in
            ScrollViewReader { proxy in
                ...
                .onAppear {
                    ...
                    olderAutoLoadArmed = false
                    initialLockToBottom = true
                    isFollowingLatest = true
                    isNearBottom = true
                    latestBottomDistance = 0
                    hasDoneInitialBottomScroll = false
                    initialBottomScrollGeneration = 0
                ...
                .onChange(of: timelineVersion) { _, _ in
                    if let olderLoadAnchorID, !isLoadingOlderTurns, displayedTimeline.contains(where: { $0.id == olderLoadAnchorID }) {
                        proxy.scrollTo(olderLoadAnchorID, anchor: .top)
                        self.olderLoadAnchorID = nil
                        return
                    }
                    if !hasDoneInitialBottomScroll || initialLockToBottom {
                        scheduleInitialBottomScroll(proxy: proxy)
                        return
                    }
    ...
    private func scheduleInitialBottomScroll(proxy: ScrollViewProxy) {
        guard initialLockToBottom else { return }
        ...
        Task { @MainActor in
            for delay in delays {
                ...
                guard generation == initialBottomScrollGeneration, initialLockToBottom else {
                    return
                }
                scrollToBottom(proxy: proxy, animated: false)
            }
            guard generation == initialBottomScrollGeneration else { return }
    ...
    private var loadOlderRow: some View {
        Button {
            requestOlderLoad(automatic: false)
        } label: {
            HStack(spacing: 8) {
                if isLoadingOlderTurns {
                    PocketDexWebSpinner(size: 12, lineWidth: 1.8, color: .white.opacity(0.68))
                } else {
                    Image(systemName: "chevron.up")
                        .font(.caption.weight(.semibold))
                }
                Text(isLoadingOlderTurns ? "Loading earlier messages" : "Load earlier messages")
                    .font(.caption.weight(.semibold))
                Spacer(minLength: 0)
            }
        ...
        .onAppear {
            requestOlderLoad(automatic: true)
        }
        .onDisappear {
            olderAutoLoadRequiresReentry = false
        }
    }

    private func requestOlderLoad(automatic: Bool) {
        guard hasOlderTurns, !isLoadingOlderTurns else { return }
        if automatic {
            guard olderAutoLoadArmed, !olderAutoLoadRequiresReentry else { return }
            olderAutoLoadRequiresReentry = true
        }
        olderLoadAnchorID = displayedTimeline.first?.id
        onLoadOlderTurns()
    }
