...
struct ThreadsListView: View {
    ...
    @State private var hasHydratedLocalCollapsedProjects = false
    @State private var collapsedProjectPersistenceSequence = 0
    ...
    private let collapsedProjectsStorageKey = "pocketdex.mobile.collapsed-projects.v1"
    ...
    var body: some View {
        NavigationStack {
            ...
            .task(id: settingsStore.serverConfiguration) {
                ...
                await hydrateCollapsedProjectsFromServer()
            ...
            .onAppear {
                hydrateLocalCollapsedProjectsIfNeeded()
            ...
            .onChange(of: collapsedProjectIDs) { _, nextProjectIDs in
                persistCollapsedProjectsLocally(nextProjectIDs)
                persistCollapsedProjectsRemotely(nextProjectIDs)
            }
    ...
    private func hydrateLocalCollapsedProjectsIfNeeded() {
        if hasHydratedLocalCollapsedProjects {
            return
        }
        hasHydratedLocalCollapsedProjects = true
        let storedProjectIDs = UserDefaults.standard.stringArray(forKey: collapsedProjectsStorageKey) ?? []
        collapsedProjectIDs = normalizeCollapsedProjectIDs(Set(storedProjectIDs))
    }

    private func persistCollapsedProjectsLocally(_ projectIDs: Set<String>) {
        let normalized = Array(normalizeCollapsedProjectIDs(projectIDs)).sorted()
        UserDefaults.standard.set(normalized, forKey: collapsedProjectsStorageKey)
    }

    private func hydrateCollapsedProjectsFromServer() async {
        guard let configuration = settingsStore.serverConfiguration else { return }
        do {
            let response = try await apiClient.fetchCollapsedProjects(config: configuration)
            let remoteProjectIDs = normalizeCollapsedProjectIDs(response.ids)
            let shouldApplyRemote =
                response.persisted ||
                (collapsedProjectIDs.isEmpty && !remoteProjectIDs.isEmpty)
            if shouldApplyRemote {
                collapsedProjectIDs = remoteProjectIDs
                rebuildProjectGroups()
                return
            }
            if !collapsedProjectIDs.isEmpty {
                persistCollapsedProjectsRemotely(collapsedProjectIDs, debounce: false)
            }
        } catch {
            // Non-fatal: local collapsed state still works offline.
        }
    }

    private func persistCollapsedProjectsRemotely(
        _ projectIDs: Set<String>,
        debounce: Bool = true
    ) {
        guard let configuration = settingsStore.serverConfiguration else { return }
        collapsedProjectPersistenceSequence += 1
        let sequence = collapsedProjectPersistenceSequence
        let normalized = normalizeCollapsedProjectIDs(projectIDs)
        Task { @MainActor in
            if debounce {
                try? await Task.sleep(nanoseconds: 220_000_000)
            }
            guard sequence == collapsedProjectPersistenceSequence else { return }
            _ = try? await apiClient.updateCollapsedProjects(normalized, config: configuration)
        }
    }

    private func normalizeCollapsedProjectIDs(_ projectIDs: Set<String>) -> Set<String> {
        Set(
            projectIDs
