...
final class PocketDexAPIClient {
    ...
    func fetchCollapsedProjects(config: ServerConfiguration) async throws -> (ids: Set<String>, persisted: Bool) {
        struct UiStateData: Decodable {
            let collapsedProjects: [String: Bool]?
        }
        ...
        return (
            ids: Self.normalizeCollapsedProjects(payload.data?.collapsedProjects),
            persisted: payload.persisted == true
        )
    }

    func updateCollapsedProjects(_ ids: Set<String>, config: ServerConfiguration) async throws -> Set<String> {
        struct UiStateData: Encodable {
            let collapsedProjects: [String: Bool]
        }
        ...
        let collapsedProjects = Dictionary(uniqueKeysWithValues: ids.map { ($0, true) })
        let payload = try await requestJSON(
            config: config,
            method: "PATCH",
            path: "/api/ui-state",
            body: Body(data: UiStateData(collapsedProjects: collapsedProjects)),
            responseType: UiStateResponse.self
        )
        return Self.normalizeCollapsedProjects(payload.data?.collapsedProjects)
    }
    ...
    ) async throws -> PocketDexThreadDetail {
        let encodedID = threadID.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? threadID
        var queryItems: [URLQueryItem] = []
        if let turnLimit, turnLimit > 0 {
            queryItems.append(URLQueryItem(name: "turnLimit", value: String(turnLimit)))
        }
        if let beforeTurnID {
            let normalized = beforeTurnID.trimmingCharacters(in: .whitespacesAndNewlines)
            if !normalized.isEmpty {
                queryItems.append(URLQueryItem(name: "beforeTurnId", value: normalized))
            }
        }
        let payload = try await requestJSON(
            config: config,
    ...
    private static func normalizeCollapsedProjects(_ record: [String: Bool]?) -> Set<String> {
        guard let record else { return [] }
        var result = Set<String>()
        for (key, value) in record where value {
