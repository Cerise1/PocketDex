...
final class PocketDexStreamClient: NSObject {
    ...
    private var pendingSubscription: (threadID: String, resume: Bool, resumeFrom: Int?, snapshotTurnLimit: Int?)?
    ...
    func subscribe(threadID: String, resume: Bool = true, resumeFrom: Int? = nil, snapshotTurnLimit: Int? = nil) {
        pendingSubscription = (threadID, resume, resumeFrom, snapshotTurnLimit)
        ...
        if let snapshotTurnLimit, snapshotTurnLimit > 0 {
            payload["snapshotTurnLimit"] = snapshotTurnLimit
        }
        send(json: payload)
    }
        ...
        Task { @MainActor [weak self] in
            ...
            if let pendingSubscription = self.pendingSubscription {
                self.subscribe(
                    ...
                    snapshotTurnLimit: pendingSubscription.snapshotTurnLimit
                )
            }
        }
    }
