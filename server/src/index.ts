...
import { resolveDefaultDebugLogDir } from "./debugLogPaths.js";
...
const COMPACT_THREAD_TITLE_MAX_CHARS = 180;
const COMPACT_THREAD_PREVIEW_MAX_CHARS = 240;
const THREAD_EVENT_HISTORY_LIMIT = 600;
const THREAD_EVENT_HISTORY_TTL_MS = 20 * 60_000;
const THREAD_SNAPSHOT_DEFAULT_TURN_LIMIT = 8;
const THREAD_READ_CACHE_TTL_MS = 2 * 60_000;
const THREAD_READ_CACHE_MAX_ENTRIES = 30;
...
const DEFAULT_DEBUG_LOG_DIR = resolveDefaultDebugLogDir(WORKSPACE_DIR);
...
class UnsupportedRequestedModelError extends Error {
  readonly requestedModel: string;
  readonly availableModels: string[];

  constructor(requestedModel: string, availableModels: string[]) {
    super(
      `Model "${requestedModel}" is not available in the local Codex runtime. Update Codex Desktop/local Codex, or choose one of: ${availableModels.join(", ")}`
    );
    this.name = "UnsupportedRequestedModelError";
...
let modelListCache: { timestamp: number; entries: ModelListEntry[] } | null = null;
const pendingThreadReadFallbackById = new Map<string, any>();
...
function truncateListText(value: unknown, maxChars: number): string {
...
function normalizeThreadListSummaryForClient(thread: any, compact: boolean): any {
  if (!compact || !thread || typeof thread !== "object") return thread;
  return {
    id: typeof thread.id === "string" ? thread.id : String(thread.id ?? ""),
    title:
      typeof thread.title === "string"
        ? truncateListText(thread.title, COMPACT_THREAD_TITLE_MAX_CHARS)
        : thread.title ?? null,
    preview: truncateListText(thread.preview, COMPACT_THREAD_PREVIEW_MAX_CHARS),
    ...
    externalRun: thread.externalRun ?? null,
  };
}
...
function modelNameForEntry(entry: ModelListEntry): string | null {
  return normalizeModelName(entry.model) ?? normalizeModelName(entry.id);
}

function modelAliasesForEntry(entry: ModelListEntry): string[] {
  const aliases = [
    normalizeModelName(entry.model),
    normalizeModelName(entry.id),
    normalizeModelName(entry.displayName),
  ].filter((value): value is string => Boolean(value));
  return Array.from(new Set(aliases));
}

async function loadAvailableModelEntries(options?: { bypassCache?: boolean }): Promise<ModelListEntry[]> {
  const now = Date.now();
  if (
    !options?.bypassCache &&
    modelListCache &&
    now - modelListCache.timestamp <= MODEL_LIST_CACHE_TTL_MS
  ) {
    return modelListCache.entries;
  }

  const result = (await bridge.request("model/list", {})) as { data?: unknown };
  const entries = Array.isArray(result?.data)
    ? result.data.filter((entry): entry is ModelListEntry => Boolean(entry && typeof entry === "object"))
    : [];
  modelListCache = { timestamp: now, entries };
  return entries;
}

function resolveModelFromEntries(requestedModel: string | null, entries: ModelListEntry[]): ModelResolution {
  if (requestedModel) {
    const normalizedRequested = requestedModel.toLowerCase();
    const match = entries.find((entry) =>
      modelAliasesForEntry(entry).some((alias) => alias.toLowerCase() === normalizedRequested)
    );
    const matchedModel = match ? modelNameForEntry(match) : null;
    if (matchedModel) {
      return {
        model: matchedModel,
        requestedModel,
  ...
  if (requestedModel) {
    const availableModels = entries
      .map((entry) => modelNameForEntry(entry))
      .filter((model): model is string => Boolean(model));
    throw new UnsupportedRequestedModelError(requestedModel, availableModels);
  }

  const defaultEntry = entries.find((entry) => entry.isDefault === true && modelNameForEntry(entry));
  const defaultModel = defaultEntry ? modelNameForEntry(defaultEntry) : null;
  if (defaultModel) {
    return {
      model: defaultModel,
      requestedModel,
  ...
  const firstVisibleEntry = entries.find((entry) => entry.hidden !== true && modelNameForEntry(entry));
  const firstVisibleModel = firstVisibleEntry ? modelNameForEntry(firstVisibleEntry) : null;
  if (firstVisibleModel) {
    return {
      model: firstVisibleModel,
      requestedModel,
...
async function resolveSupportedTurnModel(
  requestedModel: unknown,
  options: { route: "thread_start" | "message_route"; threadId?: string | null; traceId?: string | null }
): Promise<string | null> {
  const normalizedRequested = normalizeModelName(requestedModel);
  try {
    const entries = await loadAvailableModelEntries();
    const resolution = resolveModelFromEntries(normalizedRequested, entries);
    if (resolution.model !== normalizedRequested) {
      recordTitleDebug("server", "turn_model_resolved", {
    ...
    recordTitleDebug("server", "turn_model_resolution_failed", {
      threadId: options.threadId ?? null,
      detail: {
        route: options.route,
        traceId: options.traceId ?? null,
...
function normalizeThreadItemForClient(item: any): any {
  if (!item || typeof item !== "object") return item;
  if (item.type !== "fileChange") return item;
  const normalized = normalizeFileChangeItem(item);
...
function normalizeThreadTurnForClient(turn: any): any {
  if (!turn || typeof turn !== "object") return turn;
  if (!Array.isArray(turn.items)) return turn;
  return {
    ...turn,
    items: turn.items.map((item: any) => normalizeThreadItemForClient(item)),
  };
}

function normalizeThreadForClient(thread: any): any {
  if (!thread || typeof thread !== "object" || !Array.isArray(thread.turns)) return thread;
  return {
    ...thread,
    turns: thread.turns.map((turn: any) => normalizeThreadTurnForClient(turn)),
  };
}

function parsePositiveIntegerValue(value: unknown, maxValue: number): number | null {
...
function parsePositiveIntegerQuery(value: unknown, maxValue: number): number | null {
  return parsePositiveIntegerValue(value, maxValue);
}

function paginateThreadTurns(
  turns: any[],
  options: { limit: number | null; beforeTurnId: string | null }
): { turns: any[]; offset: number; total: number; hasMore: boolean } {
  const total = turns.length;
  if (!options.limit) {
    return { turns, offset: 0, total, hasMore: false };
  }

  let endExclusive = total;
  if (options.beforeTurnId) {
    const beforeIndex = turns.findIndex((turn: any) => String(turn?.id ?? "") === options.beforeTurnId);
    if (beforeIndex >= 0) endExclusive = beforeIndex;
  }

  const offset = Math.max(0, endExclusive - options.limit);
  return {
    turns: turns.slice(offset, endExclusive),
    offset,
    total,
    hasMore: offset > 0,
  };
...
type ThreadReadCacheEntry = {
  cachedAtMs: number;
  result: any;
};

const threadReadCacheById = new Map<string, ThreadReadCacheEntry>();

function getCachedThreadRead(threadId: string): any | null {
  const normalizedThreadId = normalizeThreadId(threadId);
  if (!normalizedThreadId) return null;
  const cached = threadReadCacheById.get(normalizedThreadId);
  if (!cached) return null;
  if (Date.now() - cached.cachedAtMs > THREAD_READ_CACHE_TTL_MS) {
    threadReadCacheById.delete(normalizedThreadId);
    return null;
  }
  return cached.result;
}

function rememberThreadRead(threadId: string, result: any): void {
  const normalizedThreadId = normalizeThreadId(result?.thread?.id ?? threadId);
  if (!normalizedThreadId || !result?.thread || typeof result.thread !== "object") return;
  threadReadCacheById.delete(normalizedThreadId);
  threadReadCacheById.set(normalizedThreadId, { cachedAtMs: Date.now(), result });
  while (threadReadCacheById.size > THREAD_READ_CACHE_MAX_ENTRIES) {
    const oldestKey = threadReadCacheById.keys().next().value;
    if (!oldestKey) break;
    threadReadCacheById.delete(oldestKey);
  }
}

function invalidateThreadReadCache(threadId: string | null | undefined): void {
  const normalizedThreadId = normalizeThreadId(threadId);
  if (!normalizedThreadId) return;
  threadReadCacheById.delete(normalizedThreadId);
}
...
function isThreadReadUnavailableBeforeFirstMessage(error: unknown): boolean {
...
function rememberPendingThreadReadFallback(thread: any): void {
  const threadId = normalizeThreadId(thread?.id);
  if (!threadId) return;
  pendingThreadReadFallbackById.set(
    threadId,
...
async function resolveThreadReadFallback(threadId: string, error: unknown): Promise<any | null> {
  if (!isThreadReadUnavailableBeforeFirstMessage(error)) return null;
  const normalizedThreadId = normalizeThreadId(threadId);
  if (!normalizedThreadId) return null;

  let source = "cache";
  let thread = pendingThreadReadFallbackById.get(normalizedThreadId) ?? null;
  if (!thread) {
    source = "thread_list";
    try {
      const listResult = await bridge.request("thread/list", {
        limit: 250,
        sortKey: "updated_at",
      });
      thread = Array.isArray(listResult?.data)
        ? listResult.data.find((candidate: any) => normalizeThreadId(candidate?.id) === normalizedThreadId)
        : null;
    } catch {
      thread = null;
    }
  ...
  const titles = await loadThreadTitles({ reason: "api_thread_read_fallback", threadId: normalizedThreadId });
  const title = resolveEffectiveThreadTitle(normalizedThreadId, {
    titles,
    fallbackTitle: thread?.title ?? null,
  });
  if (title) registerKnownLiveSyncTitle(normalizedThreadId, title, { source: "api_thread_read_fallback" });

  const turns = Array.isArray(thread?.turns) ? thread.turns : [];
  const externalRun = await resolveExternalRunForThreadSnapshot(normalizedThreadId, turns);
  const fallbackThread = normalizeThreadForClient({
    ...thread,
  ...
  recordTitleDebug("server", "api_thread_read_fallback", {
    threadId: normalizedThreadId,
    title,
    detail: {
      source,
...
app.get("/api/threads", async (_req, res) => {
  ...
  const compact =
    typeof _req.query.compact === "string"
      ? _req.query.compact !== "0" && _req.query.compact !== "false"
      : true;
  try {
    ...
    const enriched =
      Array.isArray(filtered)
        ...
        : filtered;
    const responseData = Array.isArray(enriched)
      ? enriched.map((thread: any) => normalizeThreadListSummaryForClient(thread, compact))
      : enriched;
    recordTitleDebug("server", "api_threads_list_response", {
      detail: {
        cursor,
        limit: Number.isFinite(limit) ? limit : null,
        archived,
    ...
    res.json({ ...result, data: responseData });
  } catch (error) {
...
app.post("/api/threads", async (req, res) => {
  ...
  try {
    const resolvedModel = await resolveSupportedTurnModel(model, { route: "thread_start" });
    ...
    if (thread?.id) {
      const normalizedThreadId = normalizeThreadId(thread.id);
      if (normalizedThreadId) touchTrackedThread(normalizedThreadId);
      rememberPendingThreadReadFallback(thread);
    }
...
app.get("/api/threads/:id", async (req, res) => {
  const { id } = req.params;
  const turnLimit = parsePositiveIntegerQuery(req.query.turnLimit, 80);
  const beforeTurnId =
    typeof req.query.beforeTurnId === "string" && req.query.beforeTurnId.trim()
      ? req.query.beforeTurnId.trim()
      : null;
  try {
    const cachedResult = beforeTurnId ? getCachedThreadRead(id) : null;
    const result = cachedResult ?? (await bridge.request("thread/read", { threadId: id, includeTurns: true }));
    if (!cachedResult) rememberThreadRead(id, result);
    const normalizedThreadId = normalizeThreadId(id);
    if (normalizedThreadId) touchTrackedThread(normalizedThreadId);
    const threadTurns = Array.isArray(result?.thread?.turns) ? result.thread.turns : [];
    const turnPage = paginateThreadTurns(threadTurns, { limit: turnLimit, beforeTurnId });
    ...
    recordTitleDebug("server", "api_thread_read_response", {
      ...
      detail: {
        ...
        returnedTurnCount: turnPage.turns.length,
        turnsOffset: turnPage.offset,
        turnsTotal: turnPage.total,
        hasMoreTurns: turnPage.hasMore,
        turnLimit,
        beforeTurnId,
      },
    });
    ...
    if (thread?.turns?.length) {
      ...
      thread = {
        ...
        turns: thread.turns.map((turn: any) => {
          ...
          return normalizeThreadTurnForClient({ ...turn, items: [...(turn.items ?? []), ...extraItems] });
        }),
      };
    }
    res.json({ ...result, thread });
  } catch (error) {
    const fallbackThread = await resolveThreadReadFallback(id, error);
    if (fallbackThread) {
      res.json({ thread: fallbackThread });
      return;
    }
    res.status(500).json({ error: errorMessageFromUnknown(error) || "Thread not found" });
  }
});
...
app.post("/api/threads/:id/messages", async (req, res) => {
  ...
  try {
    ...
    const resolvedModel = await resolveSupportedTurnModel(model, {
      route: "message_route",
      threadId,
      traceId,
    });
...
async function buildThreadSnapshot(threadId: string, options?: { turnLimit?: number | null }): Promise<any | null> {
  try {
    const cachedResult = getCachedThreadRead(threadId);
    const result = cachedResult ?? (await bridge.request("thread/read", { threadId, includeTurns: true }));
    if (!cachedResult) rememberThreadRead(threadId, result);
    ...
    const turnPage = paginateThreadTurns(threadTurns, {
      limit: options?.turnLimit ?? null,
      beforeTurnId: null,
    });
    const externalRunThreadId = normalizeThreadId(sourceThread.id ?? threadId) ?? threadId;
    const externalRun = await resolveExternalRunForThreadSnapshot(externalRunThreadId, threadTurns);
    let thread: any = normalizeThreadForClient({
      ...sourceThread,
      title,
      externalRun,
      turns: turnPage.turns,
...
async function sendThreadSnapshot(
  client: ClientState,
  threadId: string,
  options?: { reason?: "gap" | "resume" | "fallback"; seqBase?: number; turnLimit?: number | null }
): Promise<void> {
  ...
  const seqBase =
    typeof seqBaseRaw === "number" && Number.isFinite(seqBaseRaw)
      ...
      : defaultSeqBase;
  const turnLimit = options?.turnLimit ?? THREAD_SNAPSHOT_DEFAULT_TURN_LIMIT;
  const thread = await buildThreadSnapshot(threadId, { turnLimit });
  recordTitleDebug("server", "send_thread_snapshot", {
    ...
    detail: {
      ...
      seqBase,
      turnLimit,
      returnedTurnCount: Array.isArray(thread?.turns) ? thread.turns.length : 0,
      turnsTotal: typeof thread?.turnsTotal === "number" ? thread.turnsTotal : null,
...
async function sendThreadCatchUp(
  client: ClientState,
  threadId: string,
  resumeFrom: number,
  options?: { snapshotTurnLimit?: number | null }
): Promise<void> {
  const state = threadEventStateById.get(threadId);
  ...
  if (!state || state.history.length === 0) {
    if (safeResumeFrom > 0) {
      await sendThreadSnapshot(client, threadId, {
        reason: "resume",
        seqBase: 0,
        turnLimit: options?.snapshotTurnLimit ?? null,
      });
    } else {
...
function broadcastNotification(method: string, params: any): void {
  ...
  touchTrackedThread(threadId);
  invalidateThreadReadCache(threadId);
...
bridge.on("notification", (note: { method: string; params: any }) => {
  ...
  const errorPayload = note.params?.error;
  const errorObject = errorPayload && typeof errorPayload === "object" ? errorPayload : null;
  const errorMessage =
    typeof note.params?.message === "string"
      ? note.params.message
  ...
  if (
    ...
    recordTitleDebug("bridge", "notification_received", {
      ...
      detail: {
        method: note.method,
        eventTitle,
        errorName: typeof errorObject?.name === "string" ? errorObject.name : null,
        errorMessage,
        errorAdditionalDetails: errorObject?.additionalDetails ?? null,
        errorCodexInfo: errorObject?.codexErrorInfo ?? null,
...
wss.on("connection", (socket) => {
  ...
  socket.on("message", async (raw) => {
    ...
    if (msg.type === "subscribe") {
      ...
      const snapshotTurnLimit = parsePositiveIntegerValue(msg.snapshotTurnLimit, 80);
      await sendThreadCatchUp(state, threadId, resumeFrom, { snapshotTurnLimit });
      return;
    }
