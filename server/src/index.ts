...
function parseGptVersion(model: string | null): { major: number; minor: number } | null {
  if (!model) return null;
  const match = model.toLowerCase().match(/^gpt-(\d+)(?:\.(\d+))?(?:\b|-)/);
...
function gptVersionScore(version: { major: number; minor: number }): number {
  return version.major * 1_000 + version.minor;
}

function resolveNearestVersionedGptModel(requestedModel: string | null, entries: ModelListEntry[]): string | null {
  const requestedVersion = parseGptVersion(requestedModel);
  if (!requestedVersion) return null;
  const requestedScore = gptVersionScore(requestedVersion);
  const requestedLower = requestedModel?.toLowerCase() ?? "";
  const requestedIsCodex = requestedLower.includes("codex");

  const candidates = entries
    .map((entry) => {
      const model = modelNameForEntry(entry);
      const version = parseGptVersion(model);
      if (!model || !version || entry.hidden === true) return null;
      const lower = model.toLowerCase();
      if (lower.includes("mini")) return null;
      if (requestedIsCodex && !lower.includes("codex")) return null;
      return {
        model,
        score: gptVersionScore(version),
        isDefault: entry.isDefault === true,
      };
    })
    .filter((candidate): candidate is { model: string; score: number; isDefault: boolean } => Boolean(candidate));

  const notNewer = candidates
    .filter((candidate) => candidate.score <= requestedScore)
    .sort((a, b) => b.score - a.score || Number(b.isDefault) - Number(a.isDefault));
  if (notNewer[0]) return notNewer[0].model;

  const nextAvailable = candidates.sort((a, b) => a.score - b.score || Number(b.isDefault) - Number(a.isDefault));
  return nextAvailable[0]?.model ?? null;
}
...
function resolveModelFromEntries(requestedModel: string | null, entries: ModelListEntry[]): ModelResolution {
  ...
  const nearestVersionedModel = resolveNearestVersionedGptModel(requestedModel, entries);
  if (nearestVersionedModel) {
    return {
      model: nearestVersionedModel,
      requestedModel,
      reason: "nearest_version_fallback",
      availableCount: entries.length,
    };
  }
...
app.get("/api/threads/:id", async (req, res) => {
  ...
  try {
    ...
    let thread = result.thread
      ? normalizeThreadForClient({ ...result.thread, title, externalRun })
      : result.thread;
