...
const DESKTOP_HOST_ID = normalizeNonEmptyString(process.env.POCKETDEX_DESKTOP_HOST_ID) ?? "local";
...
export class DesktopLiveSync {
  ...
  private async sendSnapshot(
    ...
    if (options.nudgeThreadList) {
      broadcasts.push({
        ...
        params: desktopBroadcastParams({ conversationId: threadId }),
      });
    }
    broadcasts.push({
      method: "thread-stream-state-changed",
      version: METHOD_VERSION["thread-stream-state-changed"],
      params: desktopBroadcastParams({
  ...
  private async unlockFollowerMode(threadId: string): Promise<void> {
    ...
    try {
      recordTitleDebug("desktop_unlock_persist_archive_begin", {
        threadId,
        detail: { cwd },
      });
      await this.bridge.request("thread/archive", { threadId });
      archivePersisted = true;
      recordTitleDebug("desktop_unlock_persist_archive_success", {
        threadId,
        detail: { cwd },
      });
    } catch (error) {
      recordTitleDebug("desktop_unlock_persist_archive_failed", {
        threadId,
        detail: { cwd, error: error instanceof Error ? error.message : "unknown" },
      });
      console.warn(
        `[DesktopLiveSync] failed to persist archive for ${threadId}: ${
    ...
    await this.sendDesktopBroadcasts([
      {
        ...
        params: desktopBroadcastParams({ conversationId: threadId, cwd }),
      },
    ]);
    ...
    if (archivePersisted) {
      try {
        recordTitleDebug("desktop_unlock_persist_unarchive_begin", {
          threadId,
          detail: { gapMs: this.options.archiveUnarchiveGapMs },
        });
        await this.bridge.request("thread/unarchive", { threadId });
        recordTitleDebug("desktop_unlock_persist_unarchive_success", {
          threadId,
          detail: { gapMs: this.options.archiveUnarchiveGapMs },
        });
      } catch (error) {
        recordTitleDebug("desktop_unlock_persist_unarchive_failed", {
          threadId,
          detail: {
            gapMs: this.options.archiveUnarchiveGapMs,
            error: error instanceof Error ? error.message : "unknown",
          },
        });
        console.warn(
          `[DesktopLiveSync] failed to persist unarchive for ${threadId}: ${
            error instanceof Error ? error.message : "unknown error"
          }`
        );
      }
...
function desktopBroadcastParams(params: Record<string, unknown>): Record<string, unknown> {
  return { ...params, hostId: DESKTOP_HOST_ID };
}
