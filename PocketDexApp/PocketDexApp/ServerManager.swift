    ...
    private func resolveNodeLauncher(serverEntry: URL) throws -> (executable: URL, arguments: [String]) {
        ...
        if let explicitNode = env["POCKETDEX_NODE_BIN"], !explicitNode.isEmpty {
            ...
            if FileManager.default.isExecutableFile(atPath: nodeURL.path) {
                return (nodeURL, nodeArguments(for: nodeURL, serverEntry: serverEntry))
            }
        ...
        if let resourceURL = Bundle.main.resourceURL {
            ...
            if FileManager.default.isExecutableFile(atPath: embeddedNode.path) {
                return (embeddedNode, nodeArguments(for: embeddedNode, serverEntry: serverEntry))
            }
        }

        if let pathNode = resolveNodeFromPATH() {
            return (pathNode, nodeArguments(for: pathNode, serverEntry: serverEntry))
        }
        ...
        for candidate in commonCandidates where FileManager.default.isExecutableFile(atPath: candidate) {
            let nodeURL = URL(fileURLWithPath: candidate)
            return (nodeURL, nodeArguments(for: nodeURL, serverEntry: serverEntry))
        }

        if let voltaNode = resolveVoltaNodeBinary() {
            return (voltaNode, nodeArguments(for: voltaNode, serverEntry: serverEntry))
        }

        if let nvmNode = resolveNvmNodeBinary() {
            return (nvmNode, nodeArguments(for: nvmNode, serverEntry: serverEntry))
        }

        if let fnmNode = resolveFnmNodeBinary() {
            return (fnmNode, nodeArguments(for: fnmNode, serverEntry: serverEntry))
        }

        throw ServerManagerError.nodeBinaryNotFound
    }

    private func nodeArguments(for nodeURL: URL, serverEntry: URL) -> [String] {
        var arguments: [String] = []
        if shouldLaunchNodeJitless(nodeURL: nodeURL) {
            arguments.append("--jitless")
        }
        arguments.append(serverEntry.path)
        return arguments
    }

    private func shouldLaunchNodeJitless(nodeURL: URL) -> Bool {
        let env = ProcessInfo.processInfo.environment
        if let override = env["POCKETDEX_NODE_JITLESS"], !override.isEmpty {
            return isTruthyEnvironmentValue(override)
        }

        guard let resourceURL = Bundle.main.resourceURL else { return false }
        let embeddedNodePath = resourceURL
            .appendingPathComponent("runtime/node/bin/node")
            .standardizedFileURL
            .path
        return nodeURL.standardizedFileURL.path == embeddedNodePath
    }

    private func isTruthyEnvironmentValue(_ rawValue: String) -> Bool {
