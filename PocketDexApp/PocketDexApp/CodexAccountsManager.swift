...
final class CodexAccountsManager: ObservableObject {
    ...
    private func refreshUsageThroughCodexAppServer(for account: CodexManagedAccount) async -> CodexManagedAccount {
        ...
        do {
            ...
            let stagedAuthURL = tempHome.appendingPathComponent("auth.json")
            ...
            do {
                ...
                if let refreshedAuth = loadAuthPayload(from: stagedAuthURL) {
                ...
                let directAuth = loadAuthPayload(from: stagedAuthURL) ?? updated.auth
                let directIdentity = parseIdentity(fromIDToken: directAuth.tokens?.idToken)

                if let runtime = try? await fetchRuntimeSnapshotFromUsageEndpoint(auth: directAuth, identity: directIdentity) {
                    updated.auth = directAuth
                    updated = withRecomputedIdentity(updated)
                    updated.apply(rateLimits: runtime.rateLimits, planFallback: runtime.planType)
                    updated.usageFetchedAt = Date()
                    updated.usageCheckedAt = Date()
                    updated.lastError = nil
                    updated.isRuntimeOnly = false
                    appendDebugLog("usage_refresh_direct_success stableID=\(updated.stableID) email=\(updated.email ?? "unknown") primary=\(updated.usageSnapshot?.primary?.usedPercent ?? -1) secondary=\(updated.usageSnapshot?.secondary?.usedPercent ?? -1)")
                    return updated
                }
    ...
    private func fetchActiveRuntimeSnapshot() async -> CodexRuntimeSnapshot? {
        do {
            ...
            if let auth = loadAuthPayload(from: authFileURL) {
                let identity = parseIdentity(fromIDToken: auth.tokens?.idToken)
                if let snapshot = try? await fetchRuntimeSnapshotFromUsageEndpoint(auth: auth, identity: identity) {
                    appendDebugLog("active_runtime_snapshot_direct_success email=\(snapshot.email ?? "unknown") plan=\(snapshot.planType ?? "unknown") p=\(snapshot.rateLimits.primary?.usedPercent ?? -1) s=\(snapshot.rateLimits.secondary?.usedPercent ?? -1)")
                    return snapshot
                }
            }
    ...
    private func parseRateLimitSnapshot(from payload: [String: Any]) -> CodexRateLimitSnapshot {
        let rateLimitPayload = (payload["rate_limit"] as? [String: Any]) ?? payload
        let primaryPayload = (rateLimitPayload["primary"] as? [String: Any]) ?? (rateLimitPayload["primary_window"] as? [String: Any])
        let secondaryPayload = (rateLimitPayload["secondary"] as? [String: Any]) ?? (rateLimitPayload["secondary_window"] as? [String: Any])
        let creditsPayload = (payload["credits"] as? [String: Any]) ?? (rateLimitPayload["credits"] as? [String: Any])

        let primary = primaryPayload.flatMap(parseRateLimitWindow)
        let secondary = secondaryPayload.flatMap(parseRateLimitWindow)
        ...
        return CodexRateLimitSnapshot(
            planType: stringValue(forKeys: ["planType", "plan_type"], in: payload)
                ?? stringValue(forKeys: ["planType", "plan_type"], in: rateLimitPayload),
            primary: primary,
            secondary: secondary,
            credits: credits
        )
    ...
    private func parseRateLimitWindow(_ payload: [String: Any]) -> CodexRateLimitWindow {
        let usedPercent = numberValue(forKeys: ["usedPercent", "used_percent"], in: payload) ?? 0
        let windowSeconds = integerValue(forKeys: ["limitWindowSeconds", "limit_window_seconds"], in: payload)
        let minutes = integerValue(forKeys: ["windowDurationMins", "window_duration_mins"], in: payload)
            ?? windowSeconds.map { max(1, $0 / 60) }
        let resetEpoch = numberValue(forKeys: ["resetsAt", "resets_at", "resetAt", "reset_at"], in: payload) ?? 0
        let resetAfterSeconds = integerValue(forKeys: ["resetAfterSeconds", "reset_after_seconds"], in: payload)
        let resetAt: Date? = {
            if resetEpoch > 0 {
    ...
    private func fetchRuntimeSnapshotFromUsageEndpoint(
        auth: CodexAuthFilePayload,
        identity: CodexTokenIdentity
    ) async throws -> CodexRuntimeSnapshot {
        guard let accessToken = auth.tokens?.accessToken, !accessToken.isEmpty else {
            throw CodexAccountsError.usageEndpointFailed("Missing access token.")
        }

        guard let usageURL = resolveUsageEndpointURL() else {
            throw CodexAccountsError.usageEndpointFailed("Invalid ChatGPT base URL.")
        }
        ...
        guard let httpResponse = response as? HTTPURLResponse else {
            throw CodexAccountsError.usageEndpointFailed("Missing HTTP response.")
        }
        guard (200..<300).contains(httpResponse.statusCode) else {
            let bodySnippet = String(data: Data(data.prefix(240)), encoding: .utf8)?
                .trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
            let suffix = bodySnippet.isEmpty ? "" : ": \(bodySnippet)"
            throw CodexAccountsError.usageEndpointFailed("HTTP \(httpResponse.statusCode)\(suffix)")
        }

        guard let payload = try JSONSerialization.jsonObject(with: data) as? [String: Any] else {
            throw CodexAccountsError.appServerInvalidPayload("Usage endpoint returned malformed JSON.")
        }

        let snapshot = parseRateLimitSnapshot(from: payload)
        if snapshot.primary == nil, snapshot.secondary == nil, snapshot.credits == nil {
            throw CodexAccountsError.appServerInvalidPayload("Usage endpoint response did not contain recognizable rate limits.")
        }

        return CodexRuntimeSnapshot(
            email: stringValue(forKeys: ["email"], in: payload) ?? identity.email,
            planType: stringValue(forKeys: ["planType", "plan_type"], in: payload) ?? identity.planType,
            rateLimits: snapshot,
            isRuntimeOnlyMatch: false
        )
    }

    private func resolveUsageEndpointURL() -> URL? {
        guard var components = URLComponents(string: readChatGPTBaseURLFromConfig() ?? "https://chatgpt.com") else {
            return nil
        }
        ...
        components.path = normalizedPath + "/backend-api/wham/usage"
    ...
    private func loadCurrentAccountFromAuthFile() -> CodexManagedAccount? {
        ...
        guard let auth = loadAuthPayload(from: authFileURL) else {
            appendDebugLog("load_current_auth_file_failure error=unable_to_decode_auth")
            return nil
        }
        return accountFromAuth(auth, existingID: nil, existing: nil)
    }

    private func loadAuthPayload(from url: URL) -> CodexAuthFilePayload? {
        ...
        do {
            ...
            return try JSONDecoder().decode(CodexAuthFilePayload.self, from: data)
        } catch {
            return nil
        }
    }
...
struct CodexManagedAccount: Codable, Identifiable, Hashable {
    ...
    var displaySubtitle: String {
        ...
        if let displayPlanLabel {
            pieces.append(displayPlanLabel)
        }
        if isRuntimeOnly {
    ...
    var displayPlanLabel: String? {
        guard let planType = planType?.trimmingCharacters(in: .whitespacesAndNewlines), !planType.isEmpty else {
            return nil
        }
        return Self.localizedPlanLabel(for: planType)
    }
    ...
    private static func localizedPlanLabel(for rawPlanType: String) -> String {
...
private enum CodexAccountsError: LocalizedError {
    ...
    case usageEndpointFailed(String)
    ...
    var errorDescription: String? {
        ...
        case .usageEndpointFailed(let details):
            return "ChatGPT usage endpoint failed: \(details)"
        }
    }
}
